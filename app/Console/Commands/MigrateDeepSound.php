...
class MigrateDeepSound extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->table(
            ...
            [
                ...
                ['Artists', DB::table('tbl_artist')->count()],
    ...
    protected function migrateCategories(): void
    {
        ...
        foreach ($rows as $row) {
            ...
            $this->categoryMap[$row->id] = $row->id;
        }
    ...
    protected function migratePlaylists(): void
    {
        ...
        foreach ($playlists as $playlist) {
            ...
            $playlistContentId = DB::table('tbl_content')->insertGetId([
                'content_type' => 5,
                'channel_id' => $channelId,
                'category_id' => 1,
                'language_id' => 1,
            ...
            foreach ($playlistSongs as $ps) {
                ...
                DB::table('tbl_playlist_content')->insert([
                    'channel_id' => $channelId,
    ...
    protected function migrateLikes(): void
    {
        ...
        while (true) {
            ...
            if (!empty($insert)) {
                foreach (array_chunk($insert, 100) as $chunkData) {
                    DB::table('tbl_content_like')->insert($chunkData);
                }
            }
    ...
    protected function migrateComments(): void
    {
        ...
        $insert = [];
        foreach ($rows as $row) {
            $newUserId = $this->userMap[$row->user_id]['new_id'] ?? null;
            ...
            $insert[] = [
                ...
                'comment' => $row->value ?? '',
                'status' => 1,
                'created_at' => $row->time ? date('Y-m-d H:i:s', $row->time) : now(),
                'updated_at' => now(),
            ];
        }

        foreach (array_chunk($insert, 100) as $chunk) {
            DB::table('tbl_comment')->insert($chunk);
        }
    ...
    protected function migrateFollowers(): void
    {
        ...
        $insert = [];
        foreach ($rows as $row) {
            $newUserId = $this->userMap[$row->follower_id]['new_id'] ?? null;
            $newToUserId = $this->userMap[$row->following_id]['new_id'] ?? null;
        ...
        foreach (array_chunk($insert, 100) as $chunk) {
            DB::table('tbl_subscriber')->insert($chunk);
        }
    ...
    protected function migrateViews(): void
    {
        ...
        foreach ($rows as $row) {
            ...
            DB::table('tbl_content')->where('id', $newContentId)->increment('total_view', (int)$row->cnt);
        }

        $this->info('Migrated view counts for ' . count($rows) . ' tracks');
    }
    ...
    protected function migrateHistory(): void
    {
        ...
        foreach (array_chunk($insert, 100) as $chunk) {
            DB::table('tbl_history')->insert($chunk);
        }
