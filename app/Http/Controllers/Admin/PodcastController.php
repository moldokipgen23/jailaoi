...
class PodcastController extends Controller
{
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                if ($input_search != null && isset($input_search)) {
                    $data = Podcast::where('title', 'LIKE', "%{$input_search}%")->latest()->get();
                } else {
                    $data = Podcast::latest()->get();
                }
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('episode', function ($row) {
                        $btn = '<a href="' . route('podcast.episode.index', $row->id) . '" class="btn text-white p-1 font-weight-bold" style="background:#4e45b8;"> Episode List</a> ';
                        return $btn;
                    })
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('podcast.index')->with('success', __('Label.data_delete_successfully'));
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function PodcastDelete($podcasts_id, $id)
    {
        try {
            ...
            return redirect()->route('podcast.episode.index', $podcasts_id)->with('success', __('Label.data_delete_successfully'));
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
