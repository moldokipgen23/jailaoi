...
class VideoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $query = Content::where('content_type', 1);
            if ($input_search) {
    ...
    public function show($id)
    {
        try {
            ...
            return redirect()->route('admin.video.index')->with('success', __('label.video_delete'));
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
    ...
    public function saveChunk()
    {
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            ...
            $newFileName = 'vid' . date('_Y_m_d_') . uniqid() . '.' . $extension; // Use the extracted extension
