...
class PackageController extends Controller
{
            ...
            if ($request->ajax()) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        $delete = '<form onsubmit="return confirm(\'' . __('label.delete_package') . '\');" method="POST"  action="' . route('package.destroy', [$row->id]) . '">
                        ...
                        $btn .= '<a href="' . route('package.edit', [$row->id]) . '" class="edit-delete-btn mr-4" title=' . __('label.edit') . '>';
    ...
    public function store(Request $request)
    {
        try {
            ...
            $requestData['color'] = isset($request->color) ? $request->color : "";
            $requestData['device_limit'] = isset($request->device_limit) ? $request->device_limit : 1;
            $requestData['is_download'] = isset($request->is_download) ? $request->is_download : 0;
            ...
            if (isset($package_data->id)) {
                ...
                return response()->json(['status' => 400, 'errors' => __('label.package_not_added')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (isset($package_data->id)) {
                ...
                return response()->json(['status' => 400, 'errors' => __('label.package_not_updated')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('package.index')->with('success', __('label.success_delete_package'));
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
