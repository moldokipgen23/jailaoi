...
@section('content')
    ...
    <style>
        ...
        .jlw-title { font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:1.2px; color:var(--primary-color,#4caf50); margin-bottom:20px; display:flex; align-items:center; gap:8px; }
        .jlw-lbl  { font-size:12px; font-weight:600; color:#999; margin-bottom:6px; display:block; }
        .jlw-lbl .req { color:#f66; }
        .jlw-drop {
            border:2px dashed #363636; border-radius:14px; padding:36px 20px;
            text-align:center; cursor:pointer; position:relative; overflow:hidden; transition:border-color .22s, background .22s;
        }
        .jlw-drop:hover,.jlw-drop.dg { border-color:var(--primary-color,#4caf50); background:rgba(76,175,80,.05); }
        .jlw-drop input[type="file"] { position:absolute; inset:0; opacity:0; width:100%; height:100%; cursor:pointer; z-index:2; }
        .jlw-drop .di { font-size:34px; display:block; margin-bottom:10px; }
        .jlw-drop .dt { font-size:15px; font-weight:600; }
        .jlw-drop .ds { font-size:12px; color:#5a5a5a; margin-top:5px; }
        .jlw-curfile { font-size:12px; color:#888; margin-top:10px; font-style:italic; }
        .jlw-fname { font-size:13px; color:var(--primary-color,#4caf50); font-weight:600; margin-top:10px; display:none; word-break:break-all; }
        .jlw-wave { display:flex; align-items:flex-end; justify-content:center; gap:3px; height:26px; margin:12px auto 0; width:84px; }
        ...
        .jlw-wb:nth-child(1){animation-delay:.00s}.jlw-wb:nth-child(2){animation-delay:.10s}
        .jlw-wb:nth-child(3){animation-delay:.20s}.jlw-wb:nth-child(4){animation-delay:.15s}
        .jlw-wb:nth-child(5){animation-delay:.05s}.jlw-wb:nth-child(6){animation-delay:.25s}
        .jlw-wb:nth-child(7){animation-delay:.12s}
        @keyframes wv{0%,100%{height:3px}50%{height:22px}}
        .jlw-dur { display:none; margin-top:12px; }
        .jlw-dur-b { display:inline-flex; align-items:center; gap:7px; background:rgba(76,175,80,.1); border:1px solid var(--primary-color,#4caf50); color:var(--primary-color,#4caf50); border-radius:20px; padding:5px 14px; font-size:13px; font-weight:600; }
        .jlw-pzone { border:2px dashed #363636; border-radius:14px; padding:32px 20px; text-align:center; }
        .jlw-pzone #filelist1 { font-size:12px; color:#888; min-height:16px; margin:8px 0; }
        .jlw-pbtn { display:inline-flex; align-items:center; gap:7px; background:var(--primary-color,#4caf50); color:#fff; border-radius:22px; padding:9px 22px; font-size:13px; font-weight:600; cursor:pointer; margin-top:12px; }
        .jlw-imgz { border:2px dashed #363636; border-radius:12px; cursor:pointer; overflow:hidden; position:relative; transition:border-color .2s; display:flex; align-items:center; justify-content:center; }
        ...
        .jlw-imgz img.jlw-prev { position:absolute; inset:0; width:100%; height:100%; object-fit:cover; z-index:1; }
        .jlw-imgz input[type="file"] { position:absolute; inset:0; opacity:0; cursor:pointer; z-index:3; }
        .jlw-imgz .jlw-ie { position:absolute; bottom:8px; right:8px; z-index:4; background:rgba(0,0,0,.75); color:#fff; border-radius:6px; padding:3px 9px; font-size:11px; }
        .jlw-imgz .jlw-iov { display:none; }
        .jlw-sq { aspect-ratio:1; } .jlw-169 { aspect-ratio:16/9; }
        ...
        .jlw-bpub { background:var(--primary-color,#4caf50); color:#fff; border:none; border-radius:25px; padding:13px 34px; font-size:14px; font-weight:700; cursor:pointer; display:inline-flex; align-items:center; gap:8px; transition:all .22s; }
                ...
                <div class="jlw">
                    ...
                    <div class="jlw-steps">
                        ...
                        <div class="jlw-step">
                            ...
                            <div class="jlw-l" id="jll2">Replace Audio</div>
                        </div>
                        <div class="jlw-line" id="jlln2"></div>
                        <div class="jlw-step">
                            <div class="jlw-c" id="jlc3">3</div>
                            <div class="jlw-l" id="jll3">Artwork & Update</div>
                        </div>
                    </div>
                    ...
                    <div class="jlw-panel active" id="jlp1">
                        <div class="card custom-border-card jlw-card">
                            ...
                            <div class="form-group" id="jlf-title">
                                ...
                                <input type="text" name="title" class="form-control" value="{{ $data['title'] }}" placeholder="{{__('label.title_here')}}" autofocus>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="description" class="form-control" rows="5" placeholder="{{__('label.description_here')}}">{{ $data['description'] }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="jlw-lbl">Lyrics</label>
                                        <textarea name="lyrics" class="form-control" rows="5" placeholder="Add lyrics here...">{{ $data['lyrics'] ?? '' }}</textarea>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="jlw-nav">
                            ...
                            <button type="button" class="jlw-bnext" onclick="jlNext(1)">
                                Next: Audio &nbsp;<i class="fa-solid fa-arrow-right"></i>
                            </button>
                        </div>
                    </div>

                    {{-- ── STEP 2: Replace Audio (optional) ───── --}}
                    <div class="jlw-panel" id="jlp2">
                        <div class="card custom-border-card jlw-card">
                            <div class="jlw-title"><i class="fa-solid fa-cloud-arrow-up"></i> Replace Audio File</div>

                            {{-- Current file notice --}}
                            <div style="background:rgba(255,255,255,.04);border-radius:10px;padding:12px 16px;margin-bottom:18px;font-size:13px;display:flex;align-items:center;gap:10px">
                                <i class="fa-solid fa-circle-info" style="color:var(--primary-color,#4caf50)"></i>
                                <div>
                                    <strong>Current file:</strong> {{ basename($data['content']) }}<br>
                                    <span style="color:#777;font-size:12px">Leave blank to keep existing audio</span>
                                </div>
                            </div>
                            ...
                            <div class="mt-4" style="display:flex;align-items:center;flex-wrap:wrap;gap:16px">
                                <div>
                                    ...
                                    <input type="text" id="timePicker" name="content_duration" class="form-control" placeholder="Keep existing or auto-detect" style="width:200px">
                                </div>
                                <div class="jlw-dur" id="jl-dur">
                                    <div class="jlw-dur-b">
                                        <i class="fa-solid fa-clock"></i>
                                        <span id="jl-durval"></span>
                                        <small style="opacity:.6">new file</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="jlw-nav">
                            ...
                            <div class="jlw-sp"></div>
                            <button type="button" class="jlw-bnext" onclick="jlGo(3)">
                                Next: Artwork &amp; Update &nbsp;<i class="fa-solid fa-arrow-right"></i>
                            </button>
                        </div>
                    </div>

                    {{-- ── STEP 3: Artwork & Update ────────────── --}}
                    <div class="jlw-panel" id="jlp3">
                        ...
                        <div class="card custom-border-card jlw-card">
                            ...
                            <div class="jlw-review">
                                <div class="jlw-ri"><div class="jlw-rl">Title</div><div class="jlw-rv" id="rv-title">{{ $data['title'] }}</div></div>
                        ...
                        <div class="card custom-border-card jlw-card">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <label class="jlw-lbl mb-2">Portrait Cover <small class="text-muted">(max 5MB)</small></label>
                                    <div class="jlw-imgz jlw-sq" id="jl-pz">
                                        <img class="jlw-prev" id="imagePreview1" src="{{ $data['portrait_img'] }}" alt="">
                                        <div class="jlw-iov">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <div class="jlw-imgz jlw-169" id="jl-lz">
                                        <img class="jlw-prev" id="imagePreview2" src="{{ $data['landscape_img'] }}" alt="">
                                        <div class="jlw-iov">
                        ...
                        <div class="card custom-border-card jlw-card">
                            ...
                            <div class="jlw-togs">
                                <div class="jlw-tog">
                                    ...
                                    <label class="jlw-sw">
                                        <input type="checkbox" id="sw-c" {{ $data['is_comment'] == 1 ? 'checked' : '' }} onchange="jlSetR('is_comment',this.checked)">
                                ...
                                <div class="jlw-tog">
                                    ...
                                    <label class="jlw-sw">
                                        <input type="checkbox" id="sw-d" {{ $data['is_download'] == 1 ? 'checked' : '' }} onchange="jlSetR('is_download',this.checked)">
                                        <span class="jlw-sl"></span>
                                    </label>
                                </div>
                                <div class="jlw-tog">
                                    ...
                                    <label class="jlw-sw">
                                        <input type="checkbox" id="sw-l" {{ $data['is_like'] == 1 ? 'checked' : '' }} onchange="jlSetR('is_like',this.checked)">
                                        <span class="jlw-sl"></span>
                                    </label>
                                </div>
                            </div>
                        ...
                        <div class="card custom-border-card jlw-saving" id="jl-saving">
                            <div style="font-size:13px;font-weight:600;margin-bottom:4px">
                                <i class="fa-solid fa-cloud-arrow-up mr-2" style="color:var(--primary-color,#4caf50)"></i>Saving changes...
                            </div>
...
@section('pagescript')
    ...
    <script>
        ...
        // ── Save (original logic + XHR progress) ───────────────
        function save_music(){
