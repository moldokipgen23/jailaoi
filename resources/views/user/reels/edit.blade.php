...
@section('page_title', __('label.edit_reels'))
@section('tab_title', __('label.edit_reels'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.edit_reels')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('user.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('user.reels.index') }}">{{__('label.reels')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_reels')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('user.reels.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('label.reels_list')}}</a>
                </div>
            </div>
            ...
            <form id="reels" enctype="multipart/form-data">
                ...
                <div class="card custom-border-card mt-3">
                            ...
                            <div class="form-row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea name="title" class="form-control" rows="3" placeholder="{{__('label.title_here')}}" autofocus>{{ $data['title'] }}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="form-row">
                                    ...
                                    <div class="form-group">
                                        <div style="display: block;">
                                            ...
                                            <div id="container4" style="position: relative;">
                                                ...
                                                <label class="text-gray">{{ basename($data['content']) }}</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="border-top pt-3 text-right">
                        <button type="button" class="btn btn-default mw-120" onclick="update_reels()">{{__('label.update')}}</button>
                        <a href="{{ route('user.reels.index') }}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
		...
		function update_reels(){
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    ...
                    type:'POST',
                    url: '{{route("user.reels.update", [$data->id])}}',
                    data:formData,
                    cache:false,
                    contentType: false,
                    processData: false,
                    success:function(resp){
                        $("#dvloader").hide();
                        get_responce_message(resp, 'reels', '{{ route("user.reels.index") }}');
                    },
