...
@section('page_title', __('label.feeds'))
@section('tab_title', __('label.feeds'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.feeds')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('user.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.feeds')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('user.feed.create') }}" class="btn btn-default mw-120" style="margin-top: -14px;">{{__('label.add_feed')}}</a>
                </div>
            </div>

            <!-- Search -->
            <form action="{{ route('user.feed.index')}}" method="GET">
                        ...
                        <div class="position-relative">
                            ...
                            <ul class="list-inline overlap-control" aria-labelledby="dropdownMenuLink">
                                <li class="list-inline-item">
                                    <a class="btn" href="{{route('user.feed.edit', [$value->id])}}">
                                        <i class="fa-solid fa-pen-to-square fa-xl primary-color" class="dot-icon"></i>
                                    </a>
                                </li>
                                <li class="list-inline-item">
                                    <a class="btn" href="{{route('user.feed.show', [$value->id])}}" onclick="return confirm('{{__('label.delete_feed')}}')">
