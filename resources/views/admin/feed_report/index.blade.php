...
@section('page_title', __('label.feeds_report'))
@section('tab_title', __('label.feeds_report'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.feeds_report')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-12">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.feeds_report')}}</li>
                    </ol>
                </div>
            </div>

            <!-- Search -->
            <form action="{{ route('admin.feedreport.index')}}" method="GET">
                ...
                @foreach ($data as $key => $value)
                    <div class="col-12 col-xl-4">
                        <div class="card landscape-card">
                            <div class="media">
                                <img src="{{ $value['feed']['image'] }}" class="wallet-image">

                                <div class="card-body px-2 py-0">
                                    <h6 class="landscape-card-title">{{$value['feed']['description'] ?? ''}}</h6>
                            ...
                            <ul class="list-inline overlap-control mb-0 mt-2" aria-labelledby="dropdownMenuLink">
                                <li class="list-inline-item">
                                    @if($value['feed']['status'] == 1)
                                        <button type="button" class="show-btn" id="{{$value['feed']['id']}}" onclick="change_status({{$value['feed']['id']}})">{{__('label.show')}}</button>
                                    @elseif($value['feed']['status'] == 0)
                                        <button type="button" class="hide-btn" id="{{$value['feed']['id']}}" onclick="change_status({{$value['feed']['id']}})">{{__('label.hide')}}</button>
                                    @endif                            
                                </li>
                                @if($value['feed']['content_type'] == 2)
                                    <li class="list-inline-item">
                                        <button class="btn edit-delete-btn play-btn-top video" data-toggle="modal" data-target="#videoModal" data-video="{{ $value['feed']['video'] }}" data-image="{{ $value['feed']['image'] }}">
                                            <i class="fa-solid fa-eye fa-xl" class="dot-icon"></i>
                                        </button>
                                    </li>
                                @endif
                                <li class="list-inline-item">
                                    <a class="btn edit-delete-btn" href="{{route('admin.feedreport.show', [$value->id])}}" onclick="event.preventDefault(); confirmLink(this.href, '{{__('label.delete')}}', '{{__('label.delete_feed_report')}}')">
        ...
        function change_status(id) {
            ...
            if(Demo_Mode == 1){
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: "{{route('admin.feedreport.status')}}",
                    data: {id: id},
