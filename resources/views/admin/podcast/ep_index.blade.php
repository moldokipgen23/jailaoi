    ...
    <div class="body-content">
        ...
        <div class="border-bottom row mb-3">
            <div class="col-sm-12">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('podcast.index') }}">{{__('label.podcast')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('label.episodes')}}</li>
                </ol>
            </div>
        </div>
        <!-- search -->
        <form action="{{ route('podcast.episode.index', $podcasts_id)}}" method="GET">
            <div class="page-search mb-3">
                <div class="input-group">
                    ...
                    <input type="text" name="input_search" value="@if(isset($_GET['input_search'])){{$_GET['input_search']}}@endif" class="form-control" placeholder="{{__('label.search_episode')}}" aria-label="Search" aria-describedby="basic-addon1">
                </div>
                <div class="mr-3 ml-5">
                    <button class="btn btn-default" type="submit">{{__('label.search')}}</button>
                </div>
                <div class="mr-3 ml-5" title="Sortable">
                    <button type="button" data-toggle="modal" data-target="#exampleModal" class="btn btn-default rounded-10">
        ...
        <div class="row">
            <div class="col-12 col-sm-6 col-md-4 col-xl-3">
                <a href="{{ route('podcast.episode.add', $podcasts_id) }}" class="add-video-btn">
            ...
            <div class="col-12 col-sm-6 col-md-4 col-xl-3">
                <div class="card video-card">
                    <div class="position-relative">
                        ...
                        @if($value->episode_upload_type == 1)
                        ...
                        <ul class="list-inline overlap-control" aria-labelledby="dropdownMenuLink">
                            <li class="list-inline-item">
                                <a class="btn" href="{{route('podcast.episode.edit', [$podcasts_id, $value->id])}}" title="{{__('label.edit')}}">
                                    <i class="fa-solid fa-pen-to-square fa-xl primary-color dot-icon"></i>
                                </a>
                            </li>
                            <li class="list-inline-item">
                                <a class="btn" href="{{route('podcast.episode.delete', [$podcasts_id, $value->id])}}" title="{{__('label.delete')}}" onclick="return confirm('{{__('label.delete_episode')}}')">
...
<script>
    ...
    function save_episode_sortable() {
        ...
        if (Check_Admin == 1) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("podcast.episode.sortable") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_episode_sortable', '{{ route("podcast.episode.index", $podcasts_id)}}');
                },
