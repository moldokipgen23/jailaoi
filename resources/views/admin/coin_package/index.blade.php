...
@section('page_title', __('label.coin_package'))
@section('tab_title', __('label.coin_package'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.coin_package')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.coin_package')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('admin.coinpackage.create') }}" class="btn btn-default mw-120" style="margin-top: -14px;">{{__('label.add_coin_package')}}</a>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            var table = $('#datatable').DataTable({
                ...dataTableDefaults,
                ajax: {
                    url: "{{ route('admin.coinpackage.index') }}",
                    data: function(d) {
                ...
                columns: [
					...
					},
                    {
						...
						name: 'coin',
						render: function(data) {
                            return data ? `<span style="font-size: 18px; font-weight: 600;" class="primary-color">${data}</span>` : "0";
        ...
        function change_status(id) {
            ...
            if(Demo_Mode == 1){
                ...
                $("#dvloader").show();
                var url = `{{ route('admin.coinpackage.show', '') }}/${id}`;
